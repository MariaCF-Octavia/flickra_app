...
// Update VoiceSelector.jsx to ensure proper voice ID handling
...
// Helper to ensure we're using a valid voice ID
const getValidVoiceId = (voiceId) => {
  // If it's already a valid ID, use it
  if (FREE_TIER_VOICES.includes(voiceId)) {
    return voiceId;
  }
  
  // Otherwise default to Rachel
...
const VoiceSelector = ({ onSelect, initialVoiceId = "" }) => {
  ...
  const [selectedVoice, setSelectedVoice] = useState(
    FREE_TIER_VOICES.includes(initialVoiceId) ? initialVoiceId : "21m00Tcm4TlvDq8ikWAM"
  );
    ...
    const loadVoices = async () => {
      try {
        ...
        if (normalizedVoices.length > 0) {
          ...
          // Make sure we get a valid voice ID, not a display name
          const voiceToSelect = initialVoiceId ? 
            getValidVoiceId(initialVoiceId) : 
            getValidVoiceId(defaultVoice.id);
  ...
  const handleVoiceChange = (e) => {
    ...
    console.log(`Selected voice: ${voiceId}, using valid ID: ${validVoiceId}`);
    
    setSelectedVoice(validVoiceId);
    onSelect(validVoiceId);
  };
  ...
  const previewVoice = async () => {
    ...
    try {
      ...
      // Use the valid voice ID
      const validVoiceId = getValidVoiceId(selectedVoice);
      
      console.log(`Previewing voice ${validVoiceId}`);
      
      // Remove the /api prefix since the router already handles it
      const response = await fetch(`/elevenlabs/text-to-speech/${encodeURIComponent(validVoiceId)}`, {
        method: 'POST',
        headers: {
